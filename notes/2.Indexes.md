...
### Get the data 
Download the [speed archive](https://storage.googleapis.com/ansp-mongo-workshop/mongo/compass/speed.json.zip)
    or use a [smaller version](https://storage.googleapis.com/ansp-mongo-workshop/mongo/compass/speed_small.json.zip) if you have a smaller VM like in Mongo Atlas
Data was generated using [this script](https://raw.githubusercontent.com/ovimihai/mongodb-workshop/workshop/docker/mongo-dbs/create_data.js)
...
db.speed.find({a:{$gt:1999900}}).explain("executionStats")
```
You can now see `IXSCAN` and the name of the index used "a_1". The `executionTimeMillis` is very small and it scanned only 100 documents. 
